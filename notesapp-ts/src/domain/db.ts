...
export class NotesDB {
    ...
    open() {
        return new Promise<sqlite3.Database>((resolve, reject) => {
            try {
                const db: sqlite3.Database = new sqlite3.Database(`${process.env.DB_FILE_PATH}`, (err) => {
                    if (err) {
                        reject(err.message);
                    }
                })
    ...
    run(db: sqlite3.Database, statement: string, args: any[] = []) {
    ...
    get(db: sqlite3.Database, query: string, params: any[] = []) {
    ...
    select(db: sqlite3.Database, query: string, params: any[]= []) {
        return new Promise<any[]>((resolve, reject) => {
    ...
    async selectUserByEmail(email: string, isAuth = false) {
        try {
            const db = await this.open();
